...
impl Inspector {
    ...
    pub fn inject(&mut self, raw: RawEvent) -> Evt {
        let remove_after_enrichment = match raw.kind {
            RawEventKind::Exec {
                pid,
                ppid,
                comm,
            ...
            RawEventKind::Clone {
                ...
                self.process_table.clone_from(raw.tid, child_tid, child_pid);
                false
            }
        ...
        let evt = self.enrich(raw.tid);
        if remove_after_enrichment {
            self.process_table.remove(raw.tid);
        }
        evt
    }

    fn enrich(&self, tid: i64) -> Evt {
        let Some(thread) = self.process_table.thread(tid) else {
            return Evt::default();
        };
